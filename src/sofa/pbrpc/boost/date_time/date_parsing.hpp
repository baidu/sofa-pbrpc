#ifndef SOFA_PBRPC__DATE_TIME_DATE_PARSING_HPP___
#define SOFA_PBRPC__DATE_TIME_DATE_PARSING_HPP___
...
#include <sofa/pbrpc/boost/date_time/parse_format_base.hpp>
  ...
  convert_to_lower(std::string inp)
  {
    ...
    month_str_to_ushort(std::string const& s) {
      if((s.at(0) >= '0') && (s.at(0) <= '9')) {
        return sofa::pbrpc::boost::lexical_cast<unsigned short>(s);
      }
      else {
    ...
    parse_date(const std::string& s, int order_spec = ymd_order_iso) {
      ...
      typedef sofa::pbrpc::boost::char_separator<char, traits_type> char_separator_type;
      ...
      for(tokenizer_iterator beg=tok.begin();
        ...
        switch(spec_str.at(pos)) {
          case 'y':
          {
            year = sofa::pbrpc::boost::lexical_cast<unsigned short>(*beg);
            break;
          }
          case 'm':
          {
          ...
          case 'd':
          {
            day = sofa::pbrpc::boost::lexical_cast<unsigned short>(*beg);
            break;
          }
          default: break;
        } //switch
    ...
    parse_undelimited_date(const std::string& s) {
      ...
      sofa::pbrpc::boost::offset_separator osf(offsets, offsets+3, false, false);

      typedef typename sofa::pbrpc::boost::tokenizer<sofa::pbrpc::boost::offset_separator,
                                        std::basic_string<char>::const_iterator,
      ...
      for(typename tokenizer_type::iterator ti=tok.begin(); ti!=tok.end();++ti) {
        unsigned short i = sofa::pbrpc::boost::lexical_cast<unsigned short>(*ti);
        switch(pos) {
        case 0: y = i; break;
        case 1: m = i; break;
        case 2: d = i; break;
    ...
    from_simple_string_type(const std::basic_string<charT>& s){
      ...
      typedef typename sofa::pbrpc::boost::char_separator<charT, traits_type> char_separator;
      typedef typename sofa::pbrpc::boost::tokenizer<char_separator,
