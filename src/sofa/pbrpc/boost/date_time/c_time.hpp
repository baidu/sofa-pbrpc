#ifndef SOFA_PBRPC_DATE_TIME_C_TIME_HPP___
#define SOFA_PBRPC_DATE_TIME_C_TIME_HPP___
...
namespace date_time {
  ...
  struct c_time {
	...
	#endif
        if (!result)
          sofa::pbrpc::boost::throw_exception(std::runtime_error("could not convert calendar time to local time"));
        return result;
      }
      //! requires a pointer to a user created std::tm struct
      inline
	...
	#endif
        if (!result)
          sofa::pbrpc::boost::throw_exception(std::runtime_error("could not convert calendar time to UTC time"));
        return result;
      }
...
#endif // SOFA_PBRPC_DATE_TIME_C_TIME_HPP___
