#ifndef SOFA_PBRPC_BOOST_DATE_TIME_STRING_PARSE_TREE___HPP__
#define SOFA_PBRPC_BOOST_DATE_TIME_STRING_PARSE_TREE___HPP__
...
#include "sofa/pbrpc/boost/lexical_cast.hpp" //error without?
#include "sofa/pbrpc/boost/algorithm/string/case_conv.hpp"
#include <map>
#include <string>
#include <vector>
#include <algorithm>
...
struct string_parse_tree
{
  ...
  string_parse_tree(collection_type names, unsigned int starting_point=0)
  {
    ...
    while (index != names.size() ) {
      string_type s = sofa::pbrpc::boost::algorithm::to_lower_copy(names[index]);
