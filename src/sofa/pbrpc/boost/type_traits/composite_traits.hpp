//  (C) Copyright Dave Abrahams, Steve Cleary, Beman Dawes, Howard
//  Hinnant & John Maddock 2000.  
//  Use, modification and distribution are subject to the Boost Software License,
//  Version 1.0. (See accompanying file LICENSE_1_0.txt or copy at
//  http://www.boost.org/LICENSE_1_0.txt).
//
//  See http://www.boost.org/libs/type_traits for most recent version including documentation.
//
//  defines traits classes for composite types:
//  is_array, is_pointer, is_reference, is_member_pointer, is_enum, is_union.
//

#ifndef SOFA_PBRPC_BOOST_TT_COMPOSITE_TRAITS_HPP_INCLUDED
#define SOFA_PBRPC_BOOST_TT_COMPOSITE_TRAITS_HPP_INCLUDED

#include <sofa/pbrpc/boost/type_traits/is_array.hpp>
#include <sofa/pbrpc/boost/type_traits/is_enum.hpp>
#include <sofa/pbrpc/boost/type_traits/is_member_pointer.hpp>
#include <sofa/pbrpc/boost/type_traits/is_member_function_pointer.hpp>
#include <sofa/pbrpc/boost/type_traits/is_pointer.hpp>
#include <sofa/pbrpc/boost/type_traits/is_reference.hpp>
#include <sofa/pbrpc/boost/type_traits/is_union.hpp>

#endif // SOFA_PBRPC_BOOST_TT_COMPOSITE_TRAITS_HPP_INCLUDED





