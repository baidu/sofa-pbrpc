...
#ifndef SOFA_PBRPC_BOOST_ASIO_IMPL_BUFFERED_READ_STREAM_HPP
#define SOFA_PBRPC_BOOST_ASIO_IMPL_BUFFERED_READ_STREAM_HPP
...
template <typename Stream>
std::size_t buffered_read_stream<Stream>::fill(sofa::pbrpc::boost::system::error_code& ec)
{
  detail::buffer_resize_guard<detail::buffered_stream_storage>
    resize_guard(storage_);
  std::size_t previous_size = storage_.size();
    ...
    BOOST_ASIO_MOVE_ARG(ReadHandler) handler)
{
  ...
  if (sofa::pbrpc::boost::asio::buffer_size(buffers) == 0 || !storage_.empty())
  {
    next_layer_.async_read_some(sofa::pbrpc::boost::asio::mutable_buffers_1(0, 0),
...
#endif // SOFA_PBRPC_BOOST_ASIO_IMPL_BUFFERED_READ_STREAM_HPP
