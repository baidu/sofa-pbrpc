...
#ifndef SOFA_PBRPC_BOOST_ASIO_LOCAL_DETAIL_IMPL_ENDPOINT_IPP
#define SOFA_PBRPC_BOOST_ASIO_LOCAL_DETAIL_IMPL_ENDPOINT_IPP
...
#include <sofa/pbrpc/boost/asio/local/detail/endpoint.hpp>
...
{
  if (new_size > sizeof(sofa::pbrpc::boost::asio::detail::sockaddr_un_type))
  {
  ...
  else
  {
    path_length_ = new_size
      - offsetof(sofa::pbrpc::boost::asio::detail::sockaddr_un_type, sun_path);
...
void endpoint::init(const char* path_name, std::size_t path_length)
{
  if (path_length > sizeof(data_.local.sun_path) - 1)
  {
    ...
    sofa::pbrpc::boost::system::error_code ec(sofa::pbrpc::boost::asio::error::name_too_long);
    sofa::pbrpc::boost::asio::detail::throw_error(ec);
  }

  using namespace std; // For memcpy.
  data_.local = sofa::pbrpc::boost::asio::detail::sockaddr_un_type();
...
#endif // SOFA_PBRPC_BOOST_ASIO_LOCAL_DETAIL_IMPL_ENDPOINT_IPP
