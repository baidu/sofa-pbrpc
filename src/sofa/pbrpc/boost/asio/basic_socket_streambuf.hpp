...
#ifndef SOFA_PBRPC_BOOST_ASIO_BASIC_SOCKET_STREAMBUF_HPP
#define SOFA_PBRPC_BOOST_ASIO_BASIC_SOCKET_STREAMBUF_HPP
...
#include <sofa/pbrpc/boost/asio/detail/array.hpp>
...
#else
# include <sofa/pbrpc/boost/asio/steady_timer.hpp>
#endif

#if !defined(BOOST_ASIO_HAS_VARIADIC_TEMPLATES)

# include <sofa/pbrpc/boost/asio/detail/variadic_templates.hpp>
...
template <typename Protocol,
  ...
  || defined(GENERATING_DOCUMENTATION)
    typename Time = sofa::pbrpc::boost::posix_time::ptime,
...
public:
      ...
      const endpoint_type& endpoint)
  {
    ...
    {
      ec_ = sofa::pbrpc::boost::asio::error::operation_aborted;
      return 0;
    }
    ...
    ec_ = sofa::pbrpc::boost::asio::error::would_block;
    this->get_service().get_io_service().reset();
    do this->get_service().get_io_service().run_one();
    while (ec_ == sofa::pbrpc::boost::asio::error::would_block);
   ...
   * buffer.
   */
  const sofa::pbrpc::boost::system::error_code& puberror() const
  {
    return error();
  }
   ...
   * sofa::pbrpc::boost::asio::error::operation_aborted.
   *
...
protected:
  int_type underflow()
  {
    if (gptr() == egptr())
    {
      ...
      this->get_service().async_receive(this->get_implementation(),
          sofa::pbrpc::boost::asio::buffer(sofa::pbrpc::boost::asio::buffer(get_buffer_) + putback_max),
          0, handler);
  ...
  int_type overflow(int_type c)
  {
    if (unbuffered_)
    {
      ...
      else
      {
        ...
        this->get_service().async_send(this->get_implementation(),
            sofa::pbrpc::boost::asio::buffer(&ch, sizeof(char_type)), 0, handler);
    ...
    else
    {
      ...
      sofa::pbrpc::boost::asio::const_buffer buffer =
        sofa::pbrpc::boost::asio::buffer(pbase(), pptr() - pbase());
      while (sofa::pbrpc::boost::asio::buffer_size(buffer) > 0)
      {
        if (timer_state_ == timer_has_expired)
        {
          ec_ = sofa::pbrpc::boost::asio::error::operation_aborted;
        ...
        io_handler handler = { this };
        this->get_service().async_send(this->get_implementation(),
            sofa::pbrpc::boost::asio::buffer(buffer), 0, handler);
   ...
   * buffer.
   */
  virtual const sofa::pbrpc::boost::system::error_code& error() const
  {
    return ec_;
  }
...
private:
  ...
  void resolve_and_connect(const ResolverQuery& query)
  {
    ...
    {
      iterator_type end;
      ec_ = sofa::pbrpc::boost::asio::error::host_not_found;
      while (ec_ && i != end)
      {
        this->basic_socket<Protocol, StreamSocketService>::close(ec_);
  ...
  struct timer_handler
  {
    ...
    void operator()(const sofa::pbrpc::boost::system::error_code&)
    {
  ...
  void start_timer()
  {
    if (timer_state_ != timer_is_pending)
    {
      ...
      handler(sofa::pbrpc::boost::system::error_code());
    }
  }

  enum { putback_max = 8 };
  enum { buffer_size = 512 };
  sofa::pbrpc::boost::asio::detail::array<char, buffer_size> get_buffer_;
  sofa::pbrpc::boost::asio::detail::array<char, buffer_size> put_buffer_;
  bool unbuffered_;
  sofa::pbrpc::boost::system::error_code ec_;
...
#endif // SOFA_PBRPC_BOOST_ASIO_BASIC_SOCKET_STREAMBUF_HPP
