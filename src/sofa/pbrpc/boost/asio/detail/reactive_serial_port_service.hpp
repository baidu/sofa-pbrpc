...
#ifndef SOFA_PBRPC_BOOST_ASIO_DETAIL_REACTIVE_SERIAL_PORT_SERVICE_HPP
#define SOFA_PBRPC_BOOST_ASIO_DETAIL_REACTIVE_SERIAL_PORT_SERVICE_HPP
...
#include <sofa/pbrpc/boost/asio/detail/reactive_descriptor_service.hpp>
...
public:
  ...
  BOOST_ASIO_DECL sofa::pbrpc::boost::system::error_code open(implementation_type& impl,
...
private:
  ...
  typedef sofa::pbrpc::boost::system::error_code (*store_function_type)(
      const void*, termios&, sofa::pbrpc::boost::system::error_code&);

  // Helper function template to store a serial port option.
  template <typename SettableSerialPortOption>
  static sofa::pbrpc::boost::system::error_code store_option(const void* option,
      termios& storage, sofa::pbrpc::boost::system::error_code& ec)
  {
  ...
  BOOST_ASIO_DECL sofa::pbrpc::boost::system::error_code do_set_option(
      implementation_type& impl, store_function_type store,
      const void* option, sofa::pbrpc::boost::system::error_code& ec);

  // Function pointer type for loading a serial port option.
  typedef sofa::pbrpc::boost::system::error_code (*load_function_type)(
      void*, const termios&, sofa::pbrpc::boost::system::error_code&);

  // Helper function template to load a serial port option.
  template <typename GettableSerialPortOption>
  static sofa::pbrpc::boost::system::error_code load_option(void* option,
      const termios& storage, sofa::pbrpc::boost::system::error_code& ec)
  {
    return static_cast<GettableSerialPortOption*>(option)->load(storage, ec);
  }

  // Helper function to get a serial port option.
  BOOST_ASIO_DECL sofa::pbrpc::boost::system::error_code do_get_option(
      const implementation_type& impl, load_function_type load,
      void* option, sofa::pbrpc::boost::system::error_code& ec) const;
...
# include <sofa/pbrpc/boost/asio/detail/impl/reactive_serial_port_service.ipp>
#endif // defined(BOOST_ASIO_HEADER_ONLY)

#endif // !defined(BOOST_ASIO_WINDOWS) && !defined(__CYGWIN__)
#endif // defined(BOOST_ASIO_HAS_SERIAL_PORT)

#endif // SOFA_PBRPC_BOOST_ASIO_DETAIL_REACTIVE_SERIAL_PORT_SERVICE_HPP
